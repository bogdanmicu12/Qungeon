...
def test_full_run_settings_reveal_auto_queue_and_save_both_preferences(game):
    ...
    assert "toggle:auto_quantum_runs" not in [
        b[0] for b in game.menu.buttons()
    ]
    ...
    rects = [
        rect
        for _, rect, *_ in game.menu.buttons()
    ]

    assert all(
        pygame.Rect(0, 0, 800, 552).contains(rect)
        for rect in rects
    )
...
def test_skipping_completion_advances_without_hardware_and_keeps_final_summary(
    game,
    monkeypatch,
):
    ...
    monkeypatch.setattr(
        ...
        lambda *args: pytest.fail(
            "Automatic hardware runs are off"
        ),
    )
...
def test_auto_queue_setting_only_applies_to_full_runs_with_skipped_screens(
    game,
    monkeypatch,
    mode,
    show,
    ...
    monkeypatch.setattr(
        ...
        "command",
        lambda *args: pytest.fail(
            "No automatic request should be sent"
        ),
    )

    game.settings.update(
        full_run_completion=show,
        auto_quantum_runs=True,
    )
...
def test_auto_queue_captures_each_level_before_advancing_and_processes_in_background(
    game,
    monkeypatch,
):
    from scripts.quantum_run import QuantumRun
    ...
    assert requests[0][1] == "enqueue"
    assert first.circuit["level"] == 1
    assert first.circuit["labels"] == ["5,4"]

    assert game.current_level == 2
    assert game.quantum_run is None
...
def test_skip_screen_survives_a_broken_next_level(
    game,
    monkeypatch,
):
...
def test_level_selection_loads_and_finishes_only_that_level(
    game,
    level,
):
    click(game, "levels")
    ...
    rects = [
        slot.rect
        for slot in game.hotbar.slots.values()
    ]
...
def test_pause_freezes_hop_correlations_and_input(
    game,
    monkeypatch,
):
    game.start_level(1)
...
def test_pause_cancels_control_drag_without_spending_gate(game):
    ...
    assert not pillar.dragging
    assert pillar.control is None
...
def test_settings_round_trip_and_corrupt_file_fallback(
    tmp_path,
    monkeypatch,
):
    ...
    assert menus.load_settings() == {
        **menus.DEFAULT_SETTINGS,
        "entanglement_guides": False,
    }


def test_keyboard_navigation_and_click_release_do_not_leak_into_gameplay(
    game,
):
...
def test_toggling_a_setting_does_not_leak_into_gameplay_and_guides_can_be_disabled(
    game,
    monkeypatch,
):
    click(game, "settings")
...
def test_browser_reports_page_changes_without_repeating_each_frame(
    game,
    monkeypatch,
):
    pages = iter(
    ...
    asyncio.run(
        game.run_browser(
            on_page_change=reported.append
        )
    )
...
def spend_gate(game, name, pillar):
    """Drop a hotbar gate onto a pillar the way a player does."""
    ...
    pygame.event.post(
        pygame.event.Event(
            pygame.MOUSEBUTTONUP,
            button=1,
            pos=game.objects[pillar].rect.center,
        )
    )
...
def test_wasted_gate_prompts_but_not_immediately(
    game,
    clock,
):
    """Level 3's pillar is |->, where X is a no-op."""
    game.start_level(3)
    game.player.update_position(4, 4)
    ...
    assert game.menu.page == "playing"
    assert "X" not in game.hotbar.slots
...
def test_a_good_move_never_prompts(game, clock):
    """H is the right first move on level 3."""
    game.start_level(3)
    game.player.update_position(4, 4)
...
def test_countdown_freezes_while_paused(game, clock):
    """Pausing must not run the clock out."""
    game.start_level(3)
    game.player.update_position(4, 4)
...
def test_loot_pickup_is_checked_only_once_the_hop_lands(
    game,
    clock,
):
    """Mid-hop the player's position is fractional."""
    game.start_level(5)
...
def test_solver_runs_only_when_something_was_spent(
    game,
    clock,
    monkeypatch,
):
    """The solver is driven by gate consumption, not a timer."""
    calls = []
...
def test_failed_screen_offers_restart_settings_and_menu(game):
    """The failed prompt offers three ways out."""
    game.start_level(3)
    game.show_failed()
...
def test_restarting_from_the_prompt_clears_the_countdown(
    game,
    clock,
):
    game.start_level(3)
...
def test_stuck_detection_can_be_turned_off(
    game,
    clock,
    monkeypatch,
):
    """With the setting off, a lost level never prompts."""
    calls = []
...
def test_turning_stuck_detection_back_on_rechecks(
    game,
    clock,
):
    """Re-enabling the setting notices a level already lost."""
    game.settings["stuck_warning"] = False
...
def test_turning_stuck_detection_off_cancels_a_pending_prompt(
    game,
    clock,
):
    game.start_level(3)
    ...
    assert game.stuck_elapsed is not None
...
def test_setting_is_reachable_from_the_prompt_itself(game):
    """Settings can be opened directly from the failed prompt."""
    game.start_level(3)
    game.show_failed()
    ...
    assert any(
        action == "toggle:stuck_warning"
        for action, _, _, _ in game.menu.buttons()
    )

    click(
        game,
        "toggle:stuck_warning",
    )
