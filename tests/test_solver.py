...
def test_every_gate_the_game_offers_is_supported():
    """Fails when a gate is added to the game but not taught to the solver."""
    for name in gates:
    ...
    for name in control_gates:
        assert name in level_solver.CONTROL_EFFECTS, (
            f"controlled gate {name!r} has no entry in level_solver.CONTROL_EFFECTS; "
            "add the effect it applies to its target (see Game.handle_object_dragging)"
        )
